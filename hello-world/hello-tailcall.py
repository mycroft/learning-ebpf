...
# Note: there is an issue with generated code and recent clang version.
# See: https://github.com/iovisor/bcc/issues/4467
# Adding the -Wno-int-conversion to disable the breaking warning.
b = BPF(text=program, debug=DEBUG_PREPROCESSOR, cflags=["-Wno-int-conversion"])
...
# syscalls numbers are in /usr/include/asm/unistd_64.h
prog_array = b.get_table("syscall")
prog_array[ct.c_int(59)] = ct.c_int(exec_fn.fd) # define __NR_execve 59
prog_array[ct.c_int(222)] = ct.c_int(timer_fn.fd) # timer_create
prog_array[ct.c_int(223)] = ct.c_int(timer_fn.fd) # timer
prog_array[ct.c_int(224)] = ct.c_int(timer_fn.fd) # timer
prog_array[ct.c_int(225)] = ct.c_int(timer_fn.fd) # timer
